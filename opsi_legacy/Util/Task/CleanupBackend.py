...
from opsi_legacy.Backend.MySQL import MySQL
from opsi_legacy.Types import forceBool, forceUnicodeLower
from opsi_legacy.Util import chunk
from opsi_legacy.Util.File.Opsi import BackendDispatchConfigFile
...
BackendManager from default paths.
	:type backend: opsi_legacy.Backend.Backend
	"""
	...
	depotIds = set(depot.id for depot in backend.host_getObjects(type=["OpsiConfigserver", "OpsiDepotserver"]))
...
used MySQL backend.
	...
	with mysql.session() as session:
		...
		deleteIds = []
		found = []
		for res in mysql.getSet(session, "SELECT * FROM PRODUCT_PROPERTY_VALUE WHERE isDefault like '1'"):
			ident = ";".join(
				[
					res["propertyId"],
					res["productId"],
...
product is not existing anymore.
	...
	for productOnDepot in backend.productOnDepot_getObjects():
		...
		if productOnDepot.depotId not in depotIds:
			...
			logger.info(
				"Marking product on depot %s with missing product reference for deletion",
				productOnDepot,
			)
			deleteProductOnDepots.append(productOnDepot)
