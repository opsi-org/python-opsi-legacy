...
def _processOpsi40migrations(mysql, session):
	...
	for i in mysql.getSet(session, "SHOW TABLES;"):
		for tableName in i.values():
			...
			mysql.execute(
				session,
				"alter table `%s` convert to charset utf8 collate utf8_general_ci;" % tableName,
			)
	...
	for key in tables:
		if key.startswith("HARDWARE_DEVICE"):
			...
			for vendorId in ("NDIS", "SSTP", "AGIL", "L2TP", "PPTP", "PPPO", "PTIM"):
				mysql.execute(
					session,
					"update %s set `vendorId`=NULL where `vendorId`='%s';" % (key, vendorId),
				)
			...
			for res in mysql.getSet(session, "SELECT * FROM %s" % key):
				if res.get("vendorId"):
					try:
						...
						mysql.execute(
							session,
							"update %s set `vendorId`=NULL where `vendorId`='%s';" % (key, res["vendorId"]),
						)
				...
				if res.get("deviceId"):
					try:
						...
						mysql.execute(
							session,
							"update %s set `deviceId`=NULL where `deviceId`='%s';" % (key, res["deviceId"]),
						)
		...
		if key.startswith("HARDWARE_CONFIG") and "audit_lastseen" in tables[key]:
			...
			mysql.execute(
				session,
				"alter table %s change `audit_firstseen` `firstseen` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP;" % key,
			)
			mysql.execute(
				session,
				"alter table %s change `audit_lastseen` `lastseen` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP;" % key,
			)
			mysql.execute(
				session,
	...
	if "SOFTWARE" in tables and "name" not in tables["SOFTWARE"]:
		...
		for res in mysql.getSet(session, "SELECT * FROM `SOFTWARE`"):
			...
			res2 = mysql.getSet(
				session,
				"SELECT * FROM `SOFTWARE` where `name` = '%s' and version ='%s'" % (name, version),
			)
			if res2:
				logger.warning("Skipping duplicate: %s", res2)
				mysql.execute(
					session,
					"DELETE FROM `SOFTWARE` where `softwareId` = '%s'" % res["softwareId"].replace("'", "\\'"),
				)
				continue
	...
	if "WINDOWS_SOFTWARE_ID_TO_LICENSE_POOL" in tables:
		...
		for res in mysql.getSet(session, "SELECT * FROM `WINDOWS_SOFTWARE_ID_TO_LICENSE_POOL`"):
			res2 = mysql.getSet(
				session,
				"SELECT * FROM `SOFTWARE` where `windowsSoftwareId` = '%s'" % res["windowsSoftwareId"].replace("'", "\\'"),
			)
			if not res2:
				continue
			res2 = res2[0]
	...
	for res in mysql.getSet(session, "SELECT * FROM `LICENSE_CONTRACT`"):
		if res["licenseContractId"] != forceLicenseContractId(res["licenseContractId"]):
			...
			for res2 in mysql.getSet(
				session,
				"SELECT * FROM `SOFTWARE_LICENSE` where licenseContractId = '%s'" % deleteLicenseContractId,
			):
				...
				for tab in ("LICENSE_ON_CLIENT", "SOFTWARE_LICENSE_TO_LICENSE_POOL"):
					for res3 in mysql.getSet(
						session,
						"SELECT * FROM `%s` where softwareLicenseId = '%s'" % (tab, res2["softwareLicenseId"]),
					):
	...
	for res in mysql.getSet(session, "SELECT * FROM `LICENSE_POOL`"):
		if (res["licensePoolId"] != res["licensePoolId"].strip()) or (res["licensePoolId"] != forceLicensePoolId(res["licensePoolId"])):
			...
			for tab in (
				...
				for res2 in mysql.getSet(
					session,
					"SELECT * FROM `%s` where licensePoolId = '%s'" % (tab, deleteLicensePoolId),
				):
	...
	for res in mysql.getSet(session, "SELECT * FROM `SOFTWARE_LICENSE`"):
		if res["softwareLicenseId"] != res["softwareLicenseId"].strip() or res["softwareLicenseId"] != forceSoftwareLicenseId(
			...
			for tab in ("LICENSE_ON_CLIENT", "SOFTWARE_LICENSE_TO_LICENSE_POOL"):
				...
				for res2 in mysql.getSet(
					session,
					"SELECT * FROM `%s` where softwareLicenseId = '%s'" % (tab, deleteSoftwareLicenseId),
				):
			...
			for tableName in line.values():
				...
				for column in mysql.getSet(session, "SHOW COLUMNS FROM `%s`;" % tableName):
					...
					if "productid" in fieldName.lower() and fieldType != "varchar(255)":
						...
						mysql.execute(
							session,
							"alter table %s MODIFY COLUMN `%s` VARCHAR(255);" % (tableName, fieldName),
						)
	...
	for tableName in ("PRODUCT_PROPERTY",):
		...
		mysql.execute(
			session,
			"alter table {name} MODIFY COLUMN `{column}` TEXT;".format(name=tableName, column=fieldName),
		)
	...
	with disableForeignKeyChecks(mysql, session):
		for tablename in tables:
			if tablename == "PRODUCT_ON_DEPOT" and tableNeedsHostIDLengthFix(tablename, columnName="depotId"):
				...
				mysql.execute(
					session,
					"ALTER TABLE `{table}` MODIFY COLUMN `hostId` VARCHAR(255) NOT NULL;".format(table=tablename),
				)
		...
		for column in getTableColumns(database, session, table):
			if column.name == "licenseKey":
				...
				if length != 1024:
					...
					database.execute(
						session,
						"ALTER TABLE `{0}` MODIFY COLUMN `licenseKey` VARCHAR(1024);".format(table),
					)
...
def _adjust_length_groupid(database, session):
	...
	database.execute(
		session,
		"ALTER TABLE `OBJECT_TO_GROUP` MODIFY COLUMN `groupId` varchar(255) NOT NULL;",
	)
...
def _increase_inventory_number_length(database, session):
	...
	database.execute(
		session,
		'ALTER TABLE `HOST` MODIFY COLUMN `inventoryNumber` varchar(64) NOT NULL DEFAULT "";',
	)
