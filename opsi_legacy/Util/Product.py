...
from opsi_legacy.Types import forceBool, forceFilename, forcePackageCustomName, forceUnicode
from opsi_legacy.Util import findFilesGenerator, randomString, removeDirectory
from opsi_legacy.Util.File.Archive import Archive
from opsi_legacy.Util.File.Opsi import PackageContentFile, PackageControlFile
...
class ProductPackageSource:
	def __init__(
		...
		if customName:
			packageControlFile = os.path.join(self.packageSourceDir, f"opsi_legacy.{customName}", "control.toml")
			if not os.path.exists(packageControlFile):
				packageControlFile = os.path.join(self.packageSourceDir, f"opsi_legacy.{customName}", "control")
