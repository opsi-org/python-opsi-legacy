...
class DepotserverBackend(ExtendedBackend):
	...
	def depot_librsyncSignature(self, filename: str) -> Union[str, Any]:
		from opsi_legacy.Util.Sync import librsyncSignature
	...
	def depot_librsyncPatchFile(self, oldfile: str, deltafile: str, newfile: str) -> None:
		from opsi_legacy.Util.Sync import librsyncPatchFile
	...
	def depot_librsyncDeltaFile(self, filename: str, signature: str, deltafile: str) -> None:
	...
	def depot_createPackageContentFile(self, productId: str) -> None:
		...
		"""
		client_data_path = Path(self._context.host_getObjects(id=self._depotId)[0].getDepotLocalUrl().replace("file://", ""))
	...
	def __init__(self, depotBackend: DepotserverBackend) -> None:
		if not isinstance(depotBackend, DepotserverBackend):
			raise BackendConfigurationError(
				f"DepotserverPackageManager needs instance of DepotserverBackend as backend, got {depotBackend.__class__.__name__}"
			)
	...
	def installPackage(
		...
		def productPackageFile(filename: str, tempDir: str, depotId: str) -> Generator[ProductPackageFile, None, None]:
			...
			if not depotLocalUrl or not depotLocalUrl.startswith("file:///"):
				raise BackendBadValueError(f"Value '{depotLocalUrl}' not allowed for depot local url (has to start with 'file:///')")
		...
		def lockProduct(backend: Backend, product: str, depotId: str, forceInstallation: bool) -> ProductOnDepot:
			...
			try:
				if productOnDepots[0].getLocked():
					logger.notice(
						"Product '%s' currently locked on depot '%s'",
						productId,
						depotId,
					)
			...
			try:
				...
				logger.warning(
					"Installation error. Not unlocking product '%s' on depot '%s'.",
					productId,
					depotId,
				)
				raise err
		...
		def cleanUpProductPropertyStates(
			...
			if productPropertiesToCleanup:
				clientIds = set(clientToDepot["clientId"] for clientToDepot in backend.configState_getClientToDepotserver(depotIds=depotId))
		...
		if forceProductId:
			...
			logger.notice(
				"Installing package file '%s' as '%s' on depot '%s'",
				filename,
				forceProductId,
				depotId,
			)
		...
		try:
					...
					with lockProduct(dataBackend, product, depotId, force) as productOnDepot:
						...
						with runPackageScripts(ppf, env):
							...
							if currentProductDependencies:
								dataBackend.productDependency_deleteObjects(list(currentProductDependencies.values()))
							...
							if currentProductProperties:
								dataBackend.productProperty_deleteObjects(list(currentProductProperties.values()))

							logger.info(
								"Deleting product property states of product %s on depot '%s'",
								productId,
								depotId,
							)
							dataBackend.productPropertyState_deleteObjects(
								dataBackend.productPropertyState_getObjects(productId=productId, objectId=depotId)
							)

							logger.info(
								"Deleting not needed property states of product %s",
								productId,
							)
							...
							productPropertyStatesToDelete = [
								ppState for ppState in productPropertyStates if ppState.propertyId not in productPropertyIds
							]
							logger.debug(
								"Following productPropertyStates are marked to delete: '%s'",
								productPropertyStatesToDelete,
							)
			...
			raise BackendError(f"Failed to install package '{filename}' on depot '{depotId}': {err}") from err

	def uninstallPackage(self, productId: str, force: bool = False, deleteFiles: bool = True) -> None:
		depotId = self._depotBackend._depotId
		logger.info("=================================================================================================")
		...
		try:
			...
			try:
				allow_remove_used = forceBool(dataBackend.config_getObjects(id="allow_to_remove_package_in_use")[0].getDefaultValues()[0])
			except IndexError:
				pass

			if not allow_remove_used:
				client_ids = [
					clientToDepot["clientId"] for clientToDepot in dataBackend.configState_getClientToDepotserver(depotIds=[depotId])
				]
				if client_ids:
					productOnClients = dataBackend.productOnClient_getObjects(productId=productId, clientId=client_ids)
					if productOnClients:
						installed = 0
						action_requests = 0
			...
			try:
				...
				raise BackendBadValueError(f"Product '{productId}' is not installed on depot '{depotId}'") from err
			...
			logger.error(err, exc_info=True)
			raise BackendError(f"Failed to uninstall product '{productId}' on depot '{depotId}': {err}") from err
	...
	def checkDependencies(self, productPackageFile: ProductPackageFile) -> None:
		for dependency in productPackageFile.packageControlFile.getPackageDependencies():
			...
			if compareVersions(availableVersion, dependency["condition"], dependency["version"]):
				logger.info(
					"Fulfilled package dependency %s (available version: %s)",
					dependency,
					availableVersion,
				)
			else:
