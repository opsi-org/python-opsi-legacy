...
from opsi_legacy.Exceptions import BackendMissingDataError, BackendUnaccomplishableError
from opsi_legacy.Types import forceBool, forceHostId, forceHostIdList, forceInt, forceIpAddress, forceList, forceUnicode
...
class HostControlBackend(ExtendedBackend):
	...
	def _opsiclientdRpc(self, hostIds: list[str], method: str, params: List = None, timeout: int = None):
		if not hostIds:
	...
	def hostControl_start(self, hostIds: list[str] = None) -> dict[str, Any]:
		...
		hosts = self._context.host_getObjects(attributes=["hardwareAddress", "ipAddress"], id=hostIds or [])
		result = {}
		for host in hosts:
			try:
	...
	def hostControl_reachable(self, hostIds: list[str] = None, timeout: int = None) -> dict[str, Any]:
		hostIds = self._context.host_getIdents(id=hostIds or [], returnType="unicode")
		if not hostIds:
			raise BackendMissingDataError("No matching host ids found")
		hostIds = forceHostIdList(hostIds)
