...
from opsi_legacy.Backend.HostControlSafe import HostControlSafeBackend
...
from opsi_legacy.Types import forceBool, forceList, forceUnicodeList, forceUnicodeLowerList
...
class BackendAccessControl:
	...
	def __init__(self, backend, **kwargs):
		...
		if not self._auth_module:
			if os.name == "posix":
				...
				self._auth_module = opsi_legacy.Backend.Manager.Authentication.PAM.PAMAuthentication(pam_service)
			elif os.name == "nt":
				import opsi_legacy.Backend.Manager.Authentication.NT

				self._auth_module = opsi_legacy.Backend.Manager.Authentication.NT.NTAuthentication()
	...
	def _executeMethodProtected(self, methodName, **kwargs):
		...
		for regex, acl in self._acl:
			logger.trace("Testing if ACL pattern %s matches method %s", regex.pattern, methodName)
