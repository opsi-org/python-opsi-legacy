...
from opsi_legacy.Object import *  # needed for calls to "eval"  # noqa: F403
...
from opsi_legacy.Util import fromJson, getfqdn, toJson
from opsi_legacy.Util.File import IniFile, LockableFile
from opsi_legacy.Util.File.Opsi import HostKeyFile, PackageControlFile
...
class FileBackend(ConfigDataBackend):
		...
		if objType in ("Config", "UnicodeConfig", "BoolConfig"):
			...
			if objType == "ObjectToGroup":
				if filter.get("groupType"):
					...
					passes = [
						{
							"filename": self._getConfigFile(objType, {"groupType": "ProductGroup"}, "ini"),
							"groupType": "ProductGroup",
						},
						{
							"filename": self._getConfigFile(objType, {"groupType": "HostGroup"}, "ini"),
							"groupType": "HostGroup",
						},
					]
			else:
				if objType in ("HostGroup", "ProductGroup"):
					...
					passes = [
						{
							"filename": self._getConfigFile(objType, {"type": filter["type"]}, "ini"),
					...
					passes = [
						{
							"filename": self._getConfigFile(objType, {"type": "ProductGroup"}, "ini"),
							"groupType": "ProductGroup",
						},
						{
							"filename": self._getConfigFile(objType, {"type": "HostGroup"}, "ini"),
							"groupType": "HostGroup",
						},
					]
	...
	def _delete(self, objList: list[Any]) -> None:
		...
		if objType in ("OpsiClient", "OpsiConfigserver", "OpsiDepotserver"):
			...
			passes = [
				{
					"filename": self._getConfigFile("Group", {"type": "ProductGroup"}, "ini"),
					"groupType": "ProductGroup",
				},
				{
					"filename": self._getConfigFile("Group", {"type": "HostGroup"}, "ini"),
					"groupType": "HostGroup",
				},
			]
			for _pass in passes:
	...
	def productPropertyState_insertObject(self, productPropertyState: ProductPropertyState) -> None:
		...
		logger.debug("Inserting productPropertyState: '%s'", productPropertyState.getIdent())
	...
	def productPropertyState_updateObject(self, productPropertyState: ProductPropertyState) -> None:
		...
		logger.debug("Updating productPropertyState: '%s'", productPropertyState.getIdent())
	...
	def auditSoftwareOnClient_insertObject(self, auditSoftwareOnClient: AuditSoftwareOnClient) -> None:
		...
		logger.debug("Inserting auditSoftwareOnClient: '%s'", auditSoftwareOnClient.getIdent())
	...
	def auditSoftwareOnClient_updateObject(self, auditSoftwareOnClient: AuditSoftwareOnClient) -> None:
		...
		logger.debug("Updating auditSoftwareOnClient: '%s'", auditSoftwareOnClient.getIdent())
		filename = self._getConfigFile("AuditSoftwareOnClient", {"clientId": auditSoftwareOnClient.clientId}, "sw")
		iniFile = IniFile(filename=filename)
		ini = iniFile.parse()
	...
	def __doAuditHardwareObj(self, auditHardwareObj: Union[AuditHardware, AuditHardwareOnHost], mode: str) -> None:
