...
from opsi_legacy.Types import *  # This is needed for dynamic loading  # noqa: F403
...
class ExtendedBackend(Backend):
	...
	def _get_backend_dispatcher(self):
		from opsi_legacy.Backend.Manager.Dispatcher import BackendDispatcher
...
class ExtendedConfigDataBackend(ExtendedBackend):
	...
	def group_getIdents(self, returnType="unicode", **filter):
		return [group.getIdent(returnType) for group in self.group_getObjects(attributes=["id"], **filter)]
	...
	def licenseContract_getIdents(self, returnType="unicode", **filter):
		return [licenseContract.getIdent(returnType) for licenseContract in self.licenseContract_getObjects(attributes=["id"], **filter)]
	...
	def licensePool_getIdents(self, returnType="unicode", **filter):
		return [licensePool.getIdent(returnType) for licensePool in self.licensePool_getObjects(attributes=["id"], **filter)]
	...
	def auditHardware_getIdents(self, returnType="unicode", **filter):
		return [auditHardware.getIdent(returnType) for auditHardware in self.auditHardware_getObjects(**filter)]

	def auditHardwareOnHost_getIdents(self, returnType="unicode", **filter):
		return [auditHardwareOnHost.getIdent(returnType) for auditHardwareOnHost in self.auditHardwareOnHost_getObjects(**filter)]
