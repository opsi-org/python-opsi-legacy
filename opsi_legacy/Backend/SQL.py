...
from opsi_legacy.Types import forceBool, forceDict, forceList, forceObjectClassList, forceOpsiTimestamp, forceUnicodeList, forceUnicodeLower
...
class SQLBackend(ConfigDataBackend):
	...
	def _filterToSql(self, filter: dict[str, Any] = None, table: str = None) -> str:
		...
		def processValues(key: str, values: list[Any], table: str = None) -> Generator[str, None, None]:
			if table:
				key = f"`{table}`.`{key}`"
			else:
				key = f"`{key}`"
	...
	def _createQuery(self, table: str, attributes: list[str] = None, filter: dict[str, Any] = None) -> str:
	...
	def _adjustAttributes(self, objectClass: str, attributes: list[str], filter: dict[str, Any]):
	...
	def _objectToDatabaseHash(self, object: Any) -> dict[str, Any]:
		...
		if object.getType() == "ProductOnClient":
			...
			if fields and "isOperatingSystem" not in fields and "isOperatingSystem" in _hash:
	...
	def _objectAttributeToDatabaseAttribute(self, objectClass: str, attribute: str) -> str:
		if attribute == "id":
		...
		with self._sql.session() as session:
			...
			for hwClass, values in self._auditHardwareConfig.items():
				...
				if hardwareDeviceTableExists:
					...
					hardwareDeviceTable = (
						f"CREATE TABLE `{hardwareDeviceTableName}` (\n`hardware_id` INTEGER NOT NULL {self._sql.AUTOINCREMENT},\n"
					)
	...
	def configState_getObjects(self, attributes: list[str] = None, **filter) -> list[ConfigState]:
	...
	def product_getObjects(self, attributes: list[str] = None, **filter) -> list[Product]:
	...
	def productProperty_getObjects(self, attributes: list[str] = None, **filter) -> list[ProductProperty]:
	...
	def productDependency_getObjects(self, attributes: list[str] = None, **filter) -> list[ProductDependency]:
	...
	def productOnDepot_getObjects(self, attributes: list[str] = None, **filter) -> list[ProductOnDepot]:
	...
	def productOnClient_getObjects(self, attributes: list[str] = None, **filter) -> list[ProductOnClient]:
	...
	def productPropertyState_insertObject(self, productPropertyState: ProductPropertyState) -> None:
		...
		with self._sql.session() as session:
			if not self._sql.getSet(
				session,
				self._createQuery("HOST", ["hostId"], {"hostId": productPropertyState.objectId}),
			):
	...
	def productPropertyState_getObjects(self, attributes: list[str] = None, **filter) -> list[ProductPropertyState]:
	...
	def objectToGroup_getObjects(self, attributes: list[str] = None, **filter) -> list[ObjectToGroup]:
	...
	def licenseContract_getObjects(self, attributes: list[str] = None, **filter) -> list[LicenseContract]:
	...
	def softwareLicense_getObjects(self, attributes: list[str] = None, **filter) -> None:
	...
	def softwareLicenseToLicensePool_getObjects(self, attributes: list[str] = None, **filter) -> list[SoftwareLicenseToLicensePool]:
		...
		with self._sql.session() as session:
			return [
				...
				for res in self._sql.getSet(
					session,
					self._createQuery("SOFTWARE_LICENSE_TO_LICENSE_POOL", attributes, filter),
				)
			]
	...
	def licenseOnClient_getObjects(self, attributes: list[str] = None, **filter) -> None:
	...
	def auditSoftware_getHashes(self, attributes: list[str] = None, **filter) -> list[dict[str, Any]]:
	...
	def auditSoftware_getObjects(self, attributes: list[str] = None, **filter) -> list[AuditSoftware]:
	...
	def auditSoftwareToLicensePool_getHashes(self, attributes: list[str] = None, **filter) -> list[dict[str, Any]]:
	...
	def auditSoftwareToLicensePool_getObjects(self, attributes: list[str] = None, **filter) -> list[AuditSoftwareToLicensePool]:
		attributes = attributes or []
	...
	def auditSoftwareOnClient_getHashes(self, attributes: list[str] = None, **filter) -> list[dict[str, Any]]:
	...
	def auditSoftwareOnClient_getObjects(self, attributes: list[str] = None, **filter) -> list[AuditSoftwareOnClient]:
		attributes = attributes or []
	...
	def auditHardware_getObjects(self, attributes: list[str] = None, **filter) -> list[AuditHardware]:
		attributes = attributes or []
	...
	def auditHardware_getHashes(self, attributes: list[str] = None, **filter) -> list[dict[str, Any]]:
		return self._auditHardware_search(returnHardwareIds=False, attributes=attributes or [], **filter)

	def _auditHardware_search(self, returnHardwareIds: bool = False, attributes: list[str] = None, **filter):
		attributes = attributes or []
		hardwareClasses = set()
	...
	def auditHardwareOnHost_getHashes(self, attributes: list[str] = None, **filter) -> list[dict[str, Any]]:
		attributes = attributes or []
		hardwareClasses = set()
		hardwareClass = filter.get("hardwareClass")
		if hardwareClass not in ([], None):
	...
	def auditHardwareOnHost_getObjects(self, attributes: list[str] = None, **filter) -> list[AuditHardwareOnHost]:
