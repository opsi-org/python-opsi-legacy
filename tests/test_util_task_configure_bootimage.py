...
def testPatchMenuFileRemoveLandPwhChangeService(tempDir):
	filename = os.path.join(tempDir, 'default.menu')
	with open(filename, 'w') as writefile:
		writefile.write('label install\n')
		writefile.write('  menu label Start ^opsi bootimage\n')
	...
	expectedDefault = [
		...
		'  endtext\n',
		'  kernel install\n',
		'  append initrd=miniroot.bz2 video=vesa:ywrap,mtrr vga=791 quiet splash --no-log console=tty1 console=ttyS0 service=hhttps://8.8.8.8:4447/rpc\n',
		'\n'
	]
	with open(filename) as patchedFile:
		patchedDefault = patchedFile.readlines()
